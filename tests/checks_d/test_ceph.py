    ...
    def __init__(self, ceph_version='jewel'):
        ...
        if ceph_version == 'jewel':
            subprocess.check_output = mock.create_autospec(
                subprocess.check_output, side_effect=mocked_check_output_jewel)
        elif ceph_version == 'luminous':
            subprocess.check_output = mock.create_autospec(
                subprocess.check_output, side_effect=mocked_check_output_luminous)
...
class CephCheckTest(unittest.TestCase):
    ...
    def test_get_usage_metrics(self):
        ...
        self.assertEqual(
            { k: v for k, v in self.expected['df_total'].viewitems()
                if k != 'ceph.cluster.utilization_perc'},
            { k: v for k, v in metrics.viewitems()
                if k != 'ceph.cluster.utilization_perc'})
    ...
    def test_get_stats_metrics(self):
        ...
        self.assertEqual(self.expected['stats_metrics'], metrics)
    ...
    def test_get_osd_metrics(self):
        ...
        for osd, osd_metrics in metrics.items():
            self.assertEqual(
                { k: v for k, v in self.expected['osd_metrics'][osd].viewitems()
                    if not k in ['ceph.osd.utilization_perc', 'ceph.osd.variance']},
                { k: v for k, v in osd_metrics.viewitems()
                    if not k in ['ceph.osd.utilization_perc', 'ceph.osd.variance']})
    ...
    def test_get_osd_summary_metrics(self):
        ...
        # reliably compared with pre-computed float values
        self.assertEqual(
            { k: v for k, v in self.expected['summary_metrics'].viewitems()
    ...
    def test_get_pool_metrics(self):
        ...
        for pool, pool_metrics in metrics.iteritems():
            self.assertEqual(
                { k: v for k, v in self.expected['pool_metrics'][pool].viewitems()
                    if k != 'ceph.pool.utilization_perc'},
                { k: v for k, v in metrics[pool].viewitems()
                    if k != 'ceph.pool.utilization_perc'})
...
class CephCheckTestLuminous(CephCheckTest):
    ...
    expected = {
        ...
        'stats_metrics': {
            ...
            'ceph.cluster.pgs.avg_per_osd': 98,
